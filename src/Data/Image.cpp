...
VkFormat GetVulkanColorFormat(ImageColorFormat fmt)
{
    switch(fmt)
    {
        case ImageColorFormat::BGRA:
            return VkFormat::VK_FORMAT_B8G8R8A8_UNORM;
        case ImageColorFormat::RGBA:
            return VkFormat::VK_FORMAT_R8G8B8A8_UNORM;
        default:
            printf("Format unknown {fmt}");
            abort();
    }
}

bool CreateVulkanTexture(ImageTexture *tex_data, uint8_t *data, ImageColorFormat fmt)
{
    if (data == NULL)
        return false;
    ...
    VkFormat vulkanColorFormat = GetVulkanColorFormat(fmt);
...

    {
        ...
        VkResult err = vkCreateImage(g_Device, &info, g_Allocator, &tex_data->Image);
        check_vk_result(err);
    ...
    // Create the Image View
    {
        ...
        VkResult err = vkCreateImageView(g_Device, &info, g_Allocator, &tex_data->ImageView);
        check_vk_result(err);
    }
    ...
    // Create Sampler
    {
        ...
        VkResult err = vkCreateSampler(g_Device, &sampler_info, g_Allocator, &tex_data->Sampler);
        check_vk_result(err);
    }
...
// Helper function to cleanup an image
